...
export function HealthDashboard() {
    ...
    const renderWidget = (widget) => {
        ...
        const CommonHeader = () => (
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Stack direction="row" spacing={2} alignItems="center">
                    <Avatar variant="rounded" sx={{ bgcolor: widget.type === 'stats' ? 'primary.light' : widget.type === 'list' ? 'success.light' : 'error.light', color: widget.type === 'stats' ? 'primary.main' : widget.type === 'list' ? 'success.main' : 'error.main' }}>
                        <Icon />
                    </Avatar>
        ...
        switch (widget.type) {
            case 'info':
                return (
                    <Card key={widget.id} sx={{ height: '100%', transition: 'box-shadow 0.3s', '&:hover': { boxShadow: 4 } }}>
                        <CardContent>
                            <CommonHeader />
                            <Typography variant="body1" color="text.secondary">{widget.content}</Typography>
                        </CardContent>
                    </Card>
                );
            ...
            <Container maxWidth="lg">
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 6, flexDirection: { xs: 'column', md: 'row' }, textAlign: { xs: 'center', md: 'left' } }}>
                    <Box sx={{ mb: { xs: 2, md: 0 } }}>
                        ...
                        <Typography variant="body1" color="text.secondary">Your personalized health dashboard</Typography>
                    </Box>
                </Box>
                ...
                {dashboardConfig.customSections.map((section) => (
                    <Card key={section.id} sx={{ mb: 4, textAlign: 'center' }}>
                        <CardHeader title={section.title} titleTypographyProps={{ variant: 'h5', fontWeight: 'bold', align: 'center' }} />
                        <CardContent>
                            <Stack spacing={1.5}>
