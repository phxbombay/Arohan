...
async function saveArticlesToDatabase(articles) {
    ...
    try {
        ...
        const existingSlugsResult = await pool.query('SELECT slug FROM blogs');
        ...
        for (const article of articles) {
            try {
                ...
                // We use JSONB operator @> or just extract text if structure is simple.
                // Or simpler: check exact URL overlap relative to JSON structure.

                const existsResult = await pool.query(
                    `SELECT id FROM blogs WHERE external_source ->> 'url' = $1`,
                    [article.link]
                );
                ...
                await pool.query(
                    `INSERT INTO blogs 
                    (title, slug, content, excerpt, featured_image, category, tags, author_name, status, publish_date, external_source)
...
async function saveArticlesToDatabase(articles) {
    ...
    // Get existing slugs to avoid duplicates
    const existingSlugs = await Blog.find({}, 'slug').then(blogs => blogs.map(b => b.slug));

    for (const article of articles) {
        try {
            // Check if article already exists (by original link)
            const exists = await Blog.findOne({ 'externalSource.url': article.link });
            ...
            await Blog.create({
                ...
                status: 'published', // Auto-publish or set to 'draft' for manual review
